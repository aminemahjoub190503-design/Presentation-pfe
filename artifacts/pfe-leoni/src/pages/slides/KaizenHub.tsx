...
function Bullets({ points, dotColor }: { points: string[]; dotColor: string }) {
  return (
    <ul className="space-y-[0.7vh] text-[1.02vw] font-medium leading-snug text-ink">
      {points.map((p) => (
        <li key={p} className="flex items-start gap-[0.6vw]">
          <span className="mt-[0.65vh] h-[0.55vh] w-[0.55vh] shrink-0 rounded-full" style={{ background: dotColor }} />
          {p}
        </li>
      ))}
    </ul>
...
function Panel({
  ...
  return (
    ...
    >
      <div
        className="flex items-center justify-center py-[1.4vh] text-center font-display text-[1.4vw] font-bold text-white"
        style={{ background: headerBg }}
      >
        {title}
      </div>
      <div className="flex flex-1 flex-col p-[1.3vw]">{children}</div>
    </div>
  );
}
  ...
  return (
    <SlideShell>
      <div className="absolute left-1/2 top-[3.6vh] w-[90vw] -translate-x-1/2">
        ...
        <h2
          className="fx-rise mt-[0.6vh] font-display text-[3vw] font-bold leading-[1.02] tracking-tight text-ink"
      ...
      <div className="absolute left-1/2 top-[15vh] grid w-[90vw] -translate-x-1/2 grid-cols-3 gap-[1.8vw]">
        {/* ----- Module 4 — CI / Kaizen Hub ----- */}
        <Panel title="CI / Kaizen Hub" headerBg="#0b2150" delay="0.16s">
          <div className="rounded-[0.8vw] bg-[#f6f8fc] px-[1.1vw] py-[1.4vh] ring-1 ring-primary/8">
            <svg viewBox="0 0 280 150" className="h-[17vh] w-full" aria-hidden="true">
              {[0, 1, 2].map((g) => {
                const y = 120 - g * 45;
                return (
                  <g key={g}>
                    <line x1="34" y1={y} x2="270" y2={y} stroke="#285ccc" strokeOpacity="0.14" strokeWidth="1.2" />
                    <text x="22" y={y + 5} textAnchor="end" fontSize="14" fill="#5b6b8c" fontFamily="Manrope, sans-serif">
                      {g}
                    </text>
                  </g>
                );
              ...
              {KAIZEN_BARS.map((b, i) => {
                ...
                return (
                  <g key={b.label}>
                    ...
                    <text x={x + bw / 2} y={y - 6} textAnchor="middle" fontSize="14" fontWeight="700" fill="#0b2150" fontFamily="Space Grotesk, sans-serif">
                      {b.value}
                    </text>
        ...
        <Panel title="VSM Studio" headerBg="#285ccc" delay="0.24s">
          <div className="space-y-[1vh]">
            {VSM_STATS.map((s) => (
              <div
                key={s.label}
                className="flex items-center justify-between rounded-[0.7vw] bg-[#f6f8fc] px-[1.2vw] py-[1.5vh] ring-1 ring-primary/8"
              >
                <span className="font-display text-[1.7vw] font-bold text-primary">{s.value}</span>
                <span className="text-[1.05vw] font-semibold text-muted">{s.label}</span>
              </div>
            ))}
          </div>
        ...
        <Panel title="Portefeuille de Projets" headerBg="#7aa6ff" delay="0.32s">
          <ul className="space-y-[1.15vh]">
            {PORTFOLIO_ITEMS.map((it, i) => (
              <li key={it} className="fx-fade flex items-center gap-[0.8vw] text-[1.08vw] font-semibold text-ink" style={{ animationDelay: `${0.36 + i * 0.05}s` }}>
                <span className="h-[1.1vw] w-[1.1vw] shrink-0 rounded-[0.25vw] bg-primary" />
                {it}
              </li>
            ))}
          </ul>
          <div className="mt-auto pt-[1.8vh]">
            <div className="rounded-[0.8vw] bg-primary/[0.06] px-[1.2vw] py-[1.6vh] ring-1 ring-primary/30">
              <p className="text-center text-[1.05vw] font-semibold italic leading-snug text-primary">
